// схема Clock_IN-12 c Arduino Nano 155ИД1 и TLP627

#include <DS3231.h>
#include <Wire.h>
 
// выводы для дешифратора/катоды
  int out1 = A3;
  int out2 = A1;
  int out4 = A0;
  int out8 = A2;
// выводы для транзисторных ключей/аноды
  int key1 = 4;
  int key2 = 6;
  int key3 = 7;
  int key4 = 8;
  ...
  int sec; //переменная увеличивается каждые 500мс на единицу. Нужна для мигания секундных светодиодов, 
  //бывает в двух значениях 0 - светодиоды не горят, 1 - горят, если больше 1, то сбрысывается в 0
...
void show(int a[])
{
  ...
  setNumber(a[0]);
  if (!(mode==1&&currentdigit==0&&blinkflag==false)) //если мы в режиме настройки и происходит 
  //настройка часов, то в первая цифра будет мигать
  {
